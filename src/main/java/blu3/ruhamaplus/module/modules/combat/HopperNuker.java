...
public class HopperNuker extends Module
{
    ...
    public void onUpdate()
    {
        ...
        {
            if (this.breakingBlock != null && !RuhamaPlus.friendBlocks.containsKey(this.breakingBlock))
            {
                ...
                for (x = -4; x <= 4; ++x)
                {
                    for (int y = -4; y <= 4; ++y)
                    {
                        for (int z = -4; z <= 4; ++z)
                        {
                            ...
                            if (this.mc.world.getBlockState(pos).getBlock() instanceof BlockHopper && this.mc.world.getBlockState(pos.up()).getBlock() instanceof BlockShulkerBox && !RuhamaPlus.friendBlocks.containsKey(pos) && this.mc.player.getPositionVector().distanceTo((new Vec3d(pos)).add(0.5D, 0.5D, 0.5D)) <= 5.25D)
                            {
